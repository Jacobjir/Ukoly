package com.engeto.ukol3ifloopswitch;
...
public class Main {
    ...
    public static int sumAllInputUntilNegative() {
        // Myslím si, že tady si asi nechtěl abych použil list ale jinak mě řešení nenapadlo
        int input = Support.safeReadInt();
        int previous = 0;
        int current = input;
        int sum = 0;// Zde jsem musel hodnotu definovat jinak jsem dostával z ř.61 chybu že sum není definován. Jde to nějak abych ji nemusel *zbytečně* definovat?
        List<Integer> ints = new ArrayList<>();

        while (input >= 0) {
            current = input;
            sum = previous + current;
            System.out.println("You entered: " +input + ". Total sum: " +sum);
            ints.add(input);
            previous = input;
            input = Support.safeReadInt();
        }
